...
@Repository
public interface CashierSalesByIdRepository extends JpaRepository<Cashier, Long> {
    @Query(value = """
            WITH date_range AS (
                SELECT
            ...
            """, nativeQuery = true)
    List<CashierMonthSales> findMonthSalesById(
    ...
    @Query(value = """
            ...
            """, nativeQuery = true)
    List<CashierYearSales> findYearSalesById(
            @Param("cashierId") Long cashierId,
            @Param("year") Integer year);
}
