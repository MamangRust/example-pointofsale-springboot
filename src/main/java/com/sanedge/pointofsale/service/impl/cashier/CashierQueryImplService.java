...
public class CashierQueryImplService implements CashierQueryService {
    ...
    public ApiResponsePagination<List<CashierResponse>> findByMerchant(FindAllCashierMerchant req) {
        ...
        log.info("🔍 Finding cashiers by merchant_id={} | Page: {}, Size: {}", req.getMerchantId(), page + 1, pageSize);

        try {
            Pageable pageable = PageRequest.of(page, pageSize);
            Page<Cashier> cashierPage = cashierQueryRepository.findByMerchants(req.getMerchantId().longValue(), keyword,
                    pageable);
